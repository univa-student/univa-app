...
export function LoginForm({
    ...
    const canSubmit = useMemo(() => {
        ...
        return normalizedEmail.length > 0 && normalizedPassword.length > 0 && !isPending;
    }, [email, password, isPending]);

    return (
        <div className={cn("flex w-full flex-col items-center justify-center gap-6", className)}>
            <Card className="w-full max-w-md overflow-hidden border-0 bg-gradient-to-br from-background via-background to-muted/20 p-0 shadow-xl">
                <CardContent className="grid p-0">
                    <form onSubmit={onSubmit} className="p-8 md:p-10">
                        <FieldGroup>
                            ...
                            <Field data-invalid={Boolean(emailError) || undefined} className="mt-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className={cn(
                                            "h-11 pl-10 pr-10 transition-all duration-200 focus:ring-2 focus:ring-primary/20",
                                ...
                                {emailError && <FieldError className="mt-1 text-xs">{emailError}</FieldError>}
                            </Field>
                            ...
                            <Field data-invalid={Boolean(passwordError) || undefined}>
                                <div className="flex items-center justify-between gap-3">
                                    ...
                                    <Link
                                        to="/forgot-password"
                                        className="text-primary whitespace-nowrap text-xs underline-offset-2 transition-colors hover:underline"
                                    >
                                        Забули пароль?
                                    </Link>
                                </div>
                                ...
                                {passwordError && <FieldError className="mt-1 text-xs">{passwordError}</FieldError>}
                            </Field>
                            ...
                            <Field className="pt-2">
                                <Button
                                    ...
                                    disabled={!canSubmit}
                                    className={cn(
                                        "h-11 w-full bg-gradient-to-r from-primary to-primary/90 text-base font-medium shadow-lg transition-all duration-200 hover:from-primary/90 hover:to-primary hover:shadow-xl",
                                        !canSubmit && "cursor-not-allowed opacity-60 hover:shadow-lg"
                                    )}
                                >
                                    {isPending ? (
