...
import type { AiSummaryStyle, SummaryFlashcard } from "@/modules/ai/model/types";
import {
    ArrowLeft,
    BookOpen,
    HelpCircle,
...
function summaryStyleLabel(style?: AiSummaryStyle | null) {
    switch (style) {
        case "teacher":
            return "Пояснення як викладач";
        case "beginner":
            return "Спростити для новачка";
        default:
            return "Конспект";
    }
}

function summaryStyleIcon(style?: AiSummaryStyle | null) {
    switch (style) {
        case "teacher":
            return Brain;
        case "beginner":
...
function metaValue<T>(meta: Record<string, unknown> | undefined, camelKey: string, snakeKey: string): T | undefined {
    if (!meta) return undefined;

    return (meta[camelKey] as T | undefined) ?? (meta[snakeKey] as T | undefined);
}

function FlashcardCard({ flashcard, index }: { flashcard: SummaryFlashcard; index: number }) {
    return (
        <div className="rounded-2xl border border-border/70 bg-card p-4 shadow-sm">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                Картка {index + 1}
            </p>
            <p className="text-sm font-medium leading-6 text-foreground">{flashcard.question}</p>
            <div className="my-3 h-px bg-border/70" />
            <p className="text-sm leading-6 text-muted-foreground">{flashcard.answer}</p>
        </div>
    );
}
...
export function SummaryViewPage() {
        ...
        return (
            <div className="w-full px-4 py-6 sm:px-6">
                ...
                <div className="flex flex-col items-center gap-3 py-20 text-center">
                    ...
                    <p className="font-semibold text-foreground">Матеріал не знайдено</p>
                    <p className="text-sm text-muted-foreground">Можливо, його видалено або у вас немає доступу.</p>
                </div>
            </div>
        );
    }
    ...
    const meta = cj?.meta as Record<string, unknown> | undefined;
    const style = (cj?.style ?? meta?.style) as AiSummaryStyle | undefined;
    const StyleIcon = summaryStyleIcon(style);
    const sourceFiles = metaValue<Array<{ id: number; name: string }>>(meta, "sourceFiles", "source_files") ?? [];
    const flashcards = cj?.flashcards ?? [];
    const isMultiFile = metaValue<boolean>(meta, "isMultiFile", "is_multi_file") ?? false;
    const includeFlashcards = metaValue<boolean>(meta, "includeFlashcards", "include_flashcards") ?? false;
    const fileNames = metaValue<string[]>(meta, "fileNames", "file_names") ?? [];
    const subjectName = (metaValue<string>(meta, "subjectName", "subject_name") ?? undefined) ?? undefined;
        ...
        <div className="w-full px-4 py-6 sm:px-6">
            ...
            <div className="mb-6 rounded-[24px] border border-border/70 bg-card p-5 shadow-sm sm:p-6">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                    ...
                    {isMultiFile && (
                        <span className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-amber-500/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-amber-600 dark:text-amber-400">
                            <Layers3 className="size-3" />
                            {sourceFiles.length || metaValue<number[]>(meta, "fileIds", "file_ids")?.length || 0} файлів
                        </span>
                    )}
                    {includeFlashcards && (
                        <span className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-emerald-500/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-emerald-600 dark:text-emerald-400">
            ...
            <div className="space-y-4">
                {sourceFiles.length > 0 && (
                    <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-semibold text-foreground">Джерела</h2>
                        </div>
                ...
                {flashcards.length > 0 && (
                    <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-semibold text-foreground">Авто флеш-картки</h2>
                        </div>
                        <div className="grid gap-3 lg:grid-cols-2">
                            {flashcards.map((flashcard, index) => (
                                <FlashcardCard key={`${flashcard.question}-${index}`} flashcard={flashcard} index={index} />
                            ))}
                        </div>
                    </section>
                )}
                ...
                {cj?.possibleQuestions && cj.possibleQuestions.length > 0 && (
                    <section className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
                        ...
                        <ul className="space-y-2">
                            {cj.possibleQuestions.map((question, index) => (
                                <li
                                    key={index}
